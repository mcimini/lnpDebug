...
proof: 
  | INTROS 
  { let loc = $startpos in let lnAndCh = string_of_int loc.pos_lnum ^ "," ^ string_of_int (loc.pos_cnum - loc.pos_bol) in 
		Intros lnAndCh }
  | SEARCH 
  { let loc = $startpos in let lnAndCh = string_of_int loc.pos_lnum ^ "," ^ string_of_int (loc.pos_cnum - loc.pos_bol) in 
		Search lnAndCh }
  ...
  { let loc = $startpos in let lnAndCh = string_of_int loc.pos_lnum ^ "," ^ string_of_int (loc.pos_cnum - loc.pos_bol) in 
		  Apply(lnAndCh, name1, name2, names, instantiation) }
  | BACKCHAIN ON name = lnp_name 
  { let loc = $startpos in let lnAndCh = string_of_int loc.pos_lnum ^ "," ^ string_of_int (loc.pos_cnum - loc.pos_bol) in 
		  Backchain(lnAndCh, name) }
   | IF t = evalExp THEN p1 = proof ELSE p2 = proof option(ENDIF)
	  { let loc = $startpos in let lnAndCh = string_of_int loc.pos_lnum ^ "," ^ string_of_int (loc.pos_cnum - loc.pos_bol) in 
  		If(lnAndCh ^ "::", t, p1, p2) }
  | FOR EACH var = VAR IN t = evalExp optional_ite_info = option(for_iterating) COLON p = proof ENDFOR
  { let loc = $startpos in let lnAndCh = string_of_int loc.pos_lnum ^ "," ^ string_of_int (loc.pos_cnum - loc.pos_bol) in 
  let ite_info = if is_some optional_ite_info then get optional_ite_info else "::" in 
		  ForEachProof(lnAndCh ^ ite_info, var, t, p) }
  | p1 = proof DOT p2 = proof
      { Seq(p1, p2) }

for_iterating: 
	| ITERATING hyp = NAME AT n = INT
	{ ":" ^ hyp ^ ":" ^ (string_of_int n)}
	/*
