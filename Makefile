OCAML=ocamlfind ocamlc -w "-3-6-8-10-11-14-25-26" -g -package menhirLib -package batteries -linkpkg -thread
OUTPUT=lnp
OUTPUTDEBUG=lnpdebug
...
debug: 
	$(OCAML) language.ml $(PARSERLAN) declarationsToRules.ml pretty_printer_mod.ml debug.ml -o $(OUTPUTDEBUG)
...
clean:
	...
	rm -f $(GENERATEDDIR)*.thm.output.txt
