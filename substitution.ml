...
let rec substitution_proof proof var term = match proof with 
	...
	| NoOp -> NoOp
	| Skip -> Skip
	| Case(c, lnp_name1, lnp_name2) -> Case(c, substitution_lnp_name lnp_name1 var term, substitution_lnp_name lnp_name2 var term)
	...
	| Apply(c, lnp_name1, lnp_name2, lnp_names, inst) -> Apply(c, substitution_lnp_name lnp_name1 var term, substitution_lnp_name lnp_name2 var term, List.map (substitution_lnp_name_map_version var term) lnp_names, inst) (* inst are always literals *)
	| Backchain(c, lnp_name) -> Backchain(c, substitution_lnp_name lnp_name var term)
	| If(c, t, proof1, proof2) -> If(c, substitution_evaluatedExpression t var term, substitution_proof proof1 var term, substitution_proof proof2 var term)
	| ForEachProof(c, var2, t, proof) -> ForEachProof(c, var2, substitution_evaluatedExpression t var term, substitution_proof proof var term)
