...
let rec abella_proof (names: (string * int) list) proof = match proof with 
	...
	| Intros c -> "intros " ^ (String.concat " " (List.map fst names)) ^ ". %" ^ c ^ "\n"
	| Search c -> "search" ^ ". %" ^ c ^ "\n"
	| NoOp -> "" (* NoOp are ignored *) 
	| Skip -> "skip"  ^ ". " ^ "\n"
	(* If the name of new hypothesis is not given, then just prints "case Hyp" rather than Hyp1: case Hyp2  *)
	| Case(c, lnp_name1, lnp_name2) -> 
		let plainInstr = "case " ^ abella_lnp_name lnp_name2 ^ " (keep). %" ^ c ^ "\n " in  
	...
	| Induction(lnp_name1, lnp_name2) -> 
		let hypothesisMiddle = if print_lnp_name lnp_name1 = "_" then "" else abella_lnp_name lnp_name1 ^ ": " in 
		  "IH0 : induction on 1. " ^ "\n" ^ (abella_proof names (Intros "")) ^ hypothesisMiddle ^ "case " ^ abella_lnp_name lnp_name2 ^ " (keep). " ^ "\n" (* to be computed. Always 1 for now. "induction on " ^ abella_lnp_name lnp_name2 in *)		
	...
	| Apply(c, lnp_name1, lnp_name2, lnp_names, inst) -> 
		...
		let plainInstr = "apply " ^ abella_lnp_name lnp_name2 ^
            begin if lnp_names = [] then "" else
                " to " ^ (String.concat " " (List.map abella_lnp_name lnp_names)) end ^ with_clause ^ ". %" ^ c ^ "\n" in 
		if print_lnp_name lnp_name1 = "_" then plainInstr else abella_lnp_name lnp_name1 ^ ": " ^ plainInstr 
    (*
    ...
    *)
	| Backchain(c, lnp_name) -> "backchain " ^ abella_lnp_name lnp_name ^ ". %" ^ c ^ "\n"
	...
	| If(c, t, proof1, proof2) -> "UNRECOGNIZED: if-then-else in proof\n"
	| ForEachProof(c, var, t, proof) -> "UNRECOGNIZED: for-each iteration in proof\n"
...
let abella_thrAndProof (ForEachThm(iterationOption, lnp_name, formula, proof)) =
	if !debug_flag = true then
		...
		"UNRECOGNIZED: for each theorem" 
		else 
		"Theorem " ^ abella_lnp_name lnp_name ^ ": " ^ abella_formula formula ^ ". \n" ^ 
