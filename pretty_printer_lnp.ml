...
let rec print_evalExp t = match t with 
	...
	| Name(cname) -> cname
	| Constructor(cname, ts) -> if cname = "expBinding" then "(x)" ^ (String.concat " " (List.map print_evalExp ts)) else "(" ^ cname ^ " " ^ (String.concat " " (List.map print_evalExp ts)) ^ ")"
	...
	| EqualTerm(t1, t2) -> print_evalExp t1 ^ " = " ^ print_evalExp t2
        | OrTerm(t1, t2) ->   print_evalExp t1 ^ " or " ^ print_evalExp t2
        | IS(t1, t2) ->   print_evalExp t1 ^ " is " ^ print_evalExp t2
        | IsVar(t) -> "IsVar" ^ "(" ^ print_evalExp t ^ ")" 
        | _ -> "Operator Not Recognized"
...
let rec print_proof proof = match proof with 
	...
	| NoOp -> "noOp"
	| Skip -> "skip"
	| Case(c, lnp_name1, lnp_name2) -> print_lnp_name lnp_name1 ^ ": " ^ "case " ^ print_lnp_name lnp_name2
	...
	| Apply(c,lnp_name1, lnp_name2, lnp_names, None) -> print_lnp_name lnp_name1 ^ ": " ^ "apply " ^ print_lnp_name lnp_name2 ^ " to " ^ (String.concat " " (List.map print_lnp_name lnp_names))
	| Apply(c,lnp_name1, lnp_name2, lnp_names, Some (var1, var2)) -> print_proof (Apply(c, lnp_name1, lnp_name2, lnp_names, None)) ^ " with " ^ var1 ^ " = " ^ var2
	| Backchain(c,lnp_name) -> "backchain on " ^ print_lnp_name lnp_name
	| If(c, t, proof1, proof2) -> "if " ^ print_evalExp t ^ " then " ^ print_proof proof1 ^ " else " ^ print_proof proof2
	| ForEachProof(c, var, t, proof) -> "for each " ^ var ^ " in " ^ print_evalExp t ^ ": " ^ print_proof proof ^ " endfor"
